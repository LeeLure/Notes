      ...
      for (const [filePath, nr] of Object.entries(links)) {
        if (filePath.includes(FileBaseName)) {
          try {
            ...
            new import_obsidian.Notice(`无法获取图像文件`);
            console.error(error);
          }
        }
      }
...
var PureClearAttachment = (file, target_type, plugin) => __async(void 0, null, function* () {
  ...
  let name = target_type == "img" ? "图片" : "文件";
  try {
    if (deleteOption === ".trash") {
      yield app.vault.trash(file, false);
      new import_obsidian.Notice(name + " 移动到 Obsidian 回收站 !", SUCCESS_NOTICE_TIMEOUT);
      if (delFileFolder) {
        yield app.vault.trash(fileFolder, false);
        new import_obsidian.Notice("附件文件夹已被删除!", 3000);
      }
    } else if (deleteOption === "system-trash") {
      yield app.vault.trash(file, true);
      new import_obsidian.Notice(name + " 移动到系统回收站 !", SUCCESS_NOTICE_TIMEOUT);
      if (delFileFolder) {
      ...
      new import_obsidian.Notice(name + " 永久删除 !", SUCCESS_NOTICE_TIMEOUT);
      if (delFileFolder) {
    ...
    console.error(error);
    new import_obsidian.Notice("删除 " + name + " 失败!", SUCCESS_NOTICE_TIMEOUT);
  }
});
var handlerDelFileNew = (FileBaseName, currentMd, plugin, target_type, target_pos, in_table, in_callout) => {
  ...
  switch (state) {
    ...
    case 2:
      deleteCurTargetLink(FileBaseName, plugin, target_type, target_pos, in_table, in_callout);
      logs = refInfo.mdPath;
      new import_obsidian.Notice("由于当前文件的其他引用存在,仅删除了当前引用链接而未删除实际文件。", 3500);
    default:
      break;
  }
};
var deleteCurTargetLink = (file_base_name, plugin, target_type, target_pos, in_table, in_callout) => {
  ...
  if (!in_table && !in_callout) {
    ...
    if (finds.length == 0) {
      new import_obsidian.Notice("未能找到链接文本,请手动删除!", 0);
      return;
    } else if (finds.length != 1) {
      new import_obsidian.Notice("在当前行中找到多个相同的链接,请手动删除!", 0);
      return;
    } else {
  ...
  if (finds_all.length == 0) {
    new import_obsidian.Notice(`未能找到链接文本(对于 ${mode} 中的链接),请手动删除!`, 0);
    return;
  } else if (finds_all.length != 1) {
    new import_obsidian.Notice(`在当前 ${mode} 中找到多个相同的链接,请手动删除!`, 0);
    return;
  } else {
...
var handlerCopyFile = (FileBaseName, currentMd, plugin) => __async(void 0, null, function* () {
  ...
  try {
    ...
    new import_obsidian.Notice("已复制到剪贴板 !", SUCCESS_NOTICE_TIMEOUT);
  } catch (error) {
    console.error(error);
    new import_obsidian.Notice("复制文件失败 !", SUCCESS_NOTICE_TIMEOUT);
  }
});
...
function copyFileToClipboardCMD(filePath) {
  ...
  const callback = (error, stdout, stderr) => {
    if (error) {
      new import_obsidian.Notice(`执行命令时出错: ${error.message}`, SUCCESS_NOTICE_TIMEOUT);
...
var handlerRenameFile = (FileBaseName, currentMd, plugin) => {
  ...
  new RenameModal(plugin.app, target_folder, name.substring(0, name.length - file_type.length - 1), file_type, (result) => {
    ...
    app.vault.adapter.exists(result).then((exists) => {
      if (exists) {
        new import_obsidian.Notice(`重命名失败,因为已存在文件 ${result}`);
      } else {
...
var RenameModal = class extends import_obsidian2.Modal {
  ...
  onOpen() {
    ...
    let setting = new import_obsidian2.Setting(contentEl).setName("重命名:").addText((text) => text.setValue(this.name).onChange((value) => {
  ...
  onChooseItem(item) {
    return __async(this, null, function* () {
      ...
      if (((_a = this.target_file.parent) == null ? void 0 : _a.path) === item) {
        new import_obsidian.Notice("文件已在文件夹中!", 3000);
        return;
      }
      ...
      app.vault.adapter.exists(new_path).then((exists) => {
        if (exists) {
          new import_obsidian.Notice(`移动失败,因为已存在文件 ${new_path}`);
        } else {
...
var deleteFile = (file, plugin) => __async(void 0, null, function* () {
  ...
  try {
    ...
    console.error(error);
    new import_obsidian3.Notice("删除文件/文件夹失败 !", SUCCESS_NOTICE_TIMEOUT2);
  }
});
      ...
      for (const [filePath, nr] of Object.entries(links)) {
        ...
        try {
          ...
          if (!fileCount) {
            ...
            new import_obsidian4.Notice("所有附件及其父文件夹已删除!", 3000);
          }
        } catch (error) {
          console.warn(error);
        }
...
var AttachFlowSettingsTab = class extends import_obsidian5.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    new import_obsidian5.Setting(containerEl).setName("附件的右键菜单").setHeading();
    new import_obsidian5.Setting(containerEl).setName("已删除附件的目标位置").setDesc("选择附件在删除后要移动到的位置").addDropdown((dropdown) => {
      dropdown.addOption("permanent", "永久删除");
      dropdown.addOption(".trash", "移至Obsidian回收站");
    ...
    new import_obsidian5.Setting(containerEl).setName("移动文件到...").setDesc('在附件的右键菜单中添加一个“移动到...”选项').addToggle((toggle) => {
    ...
    new import_obsidian5.Setting(containerEl).setName("点击以查看图片").setHeading();
    new import_obsidian5.Setting(containerEl).setName("点击以查看图片").setDesc("点击图片的右侧部分以详细查看图片。").addToggle((toggle) => {
    ...
    new import_obsidian5.Setting(containerEl).setName("根据窗口大小自适应图片显示比例").setDesc("当图片尺寸超过窗口大小时,将根据窗口大小自适应显示图片。").addSlider((slider) => {
      ...
      slider.onChange((value) => __async(this, null, function* () {
        ...
        new import_obsidian5.Notice(`自适应比例: ${value}`);
        yield this.plugin.saveSettings();
      }));
      slider.setDynamicTooltip();
    });
    new import_obsidian5.Setting(containerEl).setName("拖动调整图片大小").setHeading();
    new import_obsidian5.Setting(containerEl).setName("拖动调整图片大小").setDesc("打开此选项以启用拖动调整图片大小的功能。").addToggle((toggle) => {
    ...
    new import_obsidian5.Setting(containerEl).setName("调整大小的时间间隔").setDesc("\u62D6\u62FD\u8C03\u8282\u6700\u5C0F\u523B\u5EA6\uFF08\u9ED8\u8BA4\u503C\u4E3A 0 \u5373\u4E0D\u5BF9\u9F50\u523B\u5EA6\uFF09").addText((text) => {
      text.setValue(this.plugin.settings.resizeInterval.toString()).onChange((value) => __async(this, null, function* () {
        if (value === "") {
          ...
          new import_obsidian5.Notice("请输入整数");
          text.setValue(this.plugin.settings.resizeInterval.toString());
        }
      }));
    });
    new import_obsidian5.Setting(containerEl).setName("调试").setHeading();
    new import_obsidian5.Setting(containerEl).setName("调试模式").setDesc("在控制台打印调试信息").addToggle((toggle) => {
...
var DeleteAllLogsModal = class extends import_obsidian6.Modal {
  ...
  getLog() {
    const referenceMessage = `您确定要删除 "${this.note.basename}.md" 吗？

它将被移动到您的 ${this.myPlugin.settings.deleteOption}.`;
  ...
  onOpen() {
    ...
    const referencedMessage = `当前有 [${getRefencedLinkCount()}] 个非多引用链接指向此笔记。`;
  ...
  showCancelBtn(buttonWrapper, myModal) {
    ...
    closeButton.setAttribute("aria-label", "取消操作");
  ...
  showConfirmButton(buttonWrapper, myModal) {
    ...
    removeLinkButton.setAttribute("aria-label", "继续删除当前文件及其所有非多引用附件");
      ...
      return new Promise((resolve, reject) => {
        ...
        image.onerror = () => __async(this, null, function* () {
          if (retryCount < 3) {
            ...
            reject(new Error("在三次重试后仍无法获取图像数据。"));
          }
        });
        image.src = imgSrc;
      });
...
var AttachFlowPlugin = class extends import_obsidian7.Plugin {
  constructor() {
    ...
    this.addMenuExtendedSourceMode = (menu, FileBaseName, currentMd, target_type, target_pos, inTable, inCallout) => {
      ...
      menu.addItem((item) => item.setIcon("pencil").setTitle("重命名").onClick(() => __async(this, null, function* () {
        try {
          print("test rename");
          handlerRenameFile(FileBaseName, currentMd, this);
        } catch (e) {
          new import_obsidian7.Notice("错误,无法重命名文件!");
        }
      })));
      if (this.settings.moveFileMenu) {
        menu.addItem((item) => item.setIcon("folder-tree").setTitle("移动文件到...").onClick(() => __async(this, null, function* () {
          try {
            handlerMoveFile(FileBaseName, currentMd, this);
          } catch (e) {
            new import_obsidian7.Notice("错误,无法移动文件!");
          }
        })));
      }
      menu.addItem((item) => item.setIcon("trash-2").setTitle("清除文件及关联链接").onClick(() => __async(this, null, function* () {
        try {
          handlerDelFileNew(FileBaseName, currentMd, this, target_type, target_pos, inTable, inCallout);
        } catch (e) {
          new import_obsidian7.Notice("错误,无法清空文件!");
        }
      })));
    };
    this.addMenuExtendedPreviewMode = (menu, FileBaseName, currentMd) => {
      ...
      if (process.platform != "linux") {
        menu.addItem((item) => item.setIcon("copy").setTitle("复制文件到剪贴板").onClick(() => __async(this, null, function* () {
          try {
            handlerCopyFile(FileBaseName, currentMd, this);
          } catch (e) {
            new import_obsidian7.Notice("错误,无法复制文件!");
          }
        })));
      }
      menu.addItem((item) => item.setIcon("arrow-up-right").setTitle("使用默认应用程序打开").onClick(() => this.app.openWithDefaultApp(file.path)));
      menu.addItem((item) => item.setIcon("arrow-up-right").setTitle(import_obsidian7.Platform.isMacOS ? "在查找器中显示" : "在系统资源管理器中显示").onClick(() => {
        this.app.showInFolder(file.path);
      }));
      menu.addItem((item) => item.setIcon("folder").setTitle("在导航中显示文件").onClick(() => {
    ...
    this.addExternalImageMenuPreviewMode = (menu, img) => {
      menu.addItem((item) => item.setIcon("copy").setTitle("复制图片到剪贴板").onClick(() => __async(this, null, function* () {
        try {
          const blob = yield loadImageBlob(img.src);
          yield navigator.clipboard.write([new ClipboardItem({ "image/png": blob })]);
          new import_obsidian7.Notice("图像已复制到剪贴板");
        } catch (error) {
          new import_obsidian7.Notice("复制图像失败!");
        }
      })));
      menu.addItem((item) => item.setIcon("link").setTitle("复制图片链接").onClick(() => __async(this, null, function* () {
        navigator.clipboard.writeText(img.src);
      })));
      menu.addItem((item) => item.setIcon("link").setTitle("复制Markdown链接").onClick(() => __async(this, null, function* () {
        navigator.clipboard.writeText(`![](${img.src})`);
      })));
      menu.addItem((item) => item.setIcon("external-link").setTitle("在外置浏览器中打开").onClick(() => __async(this, null, function* () {
        window.open(img.src, "_blank");
      })));
    };
    this.addExternalImageMenuSourceMode = (menu, img, inTable, inCallout) => {
      this.addExternalImageMenuPreviewMode(menu, img);
      menu.addItem((item) => item.setIcon("trash-2").setTitle("清除图片链接").onClick(() => {
        var _a;
      ...
      this.registerEvent(this.app.workspace.on("file-menu", (menu, file) => {
        if (file instanceof import_obsidian7.TFile) {
          ...
          const addMenuItem = (item) => {
            item.setTitle("删除文件及其附件").setIcon("trash-2").setSection("danger");
...
function updateInternalLink(activeView, target, target_pos, imageName, newWidth, newHeight, inTable, inCallout) {
  ...
  if (!inCallout && !inTable) {
    ...
    if (matched.length == 1) {
      ...
      new import_obsidian7.Notice("在当前行中找到多个相同的图像链接,请手动缩放!");
    }
    return;
  }
  let startReg = {
  ...
  if (matched_results.length == 1) {
    ...
    new import_obsidian7.Notice(`在 ${mode} 中未能找到当前图像链接,请手动缩放!`);
  } else {
    new import_obsidian7.Notice(`在 ${mode} 中找到多个相同的图像链接,请手动缩放!`);
  }
  return;
}
