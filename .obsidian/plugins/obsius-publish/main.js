...
var ObsiusPlugin = class extends import_obsidian2.Plugin {
  ...
  async publishFile(file) {
    try {
      ...
      new import_obsidian2.Notice(getText("动作.创建.成功"));
    } catch (e) {
      console.error(e);
      new import_obsidian2.Notice(getText("动作.创建.失败"));
    }
  }
  async updateFile(file) {
    try {
      await this.obsiusClient.updatePost(file);
      new import_obsidian2.Notice(getText("操作.更新.成功"));
    } catch (e) {
      console.error(e);
      new import_obsidian2.Notice(getText("操作.更新.失败"));
    }
  }
  async copyUrl(file) {
    ...
    if (url) {
      await navigator.clipboard.writeText(url);
      new import_obsidian2.Notice(getText("动作.copyUrl.成功"));
    } else {
      new import_obsidian2.Notice(getText("操作.copyUrl.失败"));
    }
  }
  async deleteFile(file) {
    try {
      await this.obsiusClient.deletePost(file);
      new import_obsidian2.Notice(getText("操作.删除.成功"));
    } catch (e) {
      console.error(e);
      new import_obsidian2.Notice(getText("操作.删除.失败"));
    }
  }
};
